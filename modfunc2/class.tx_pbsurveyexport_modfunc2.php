...
class tx_pbsurveyexport_modfunc2 extends \TYPO3\CMS\Backend\Module\AbstractFunctionModule {
	...
	private function indexAction() {
		...
		$lines[] = '<input type="submit" name="' . $this->pObj->strExtKey . '[submit]" value="' . $LANG->getLL('submitDelete') . '" />';
		$lines[] = '</fieldset>';
	...
	private function confirmationAction() {
		...
		$lines[] = '<input type="submit" name="' . $this->pObj->strExtKey . '[submit]" value="' . $LANG->getLL('submitConfirmation') . '" />';
		$lines[] = '</fieldset>';
	...
	private function deleteAction() {
		...
		if ($this->deleteAnswers() && $this->deleteResults()) {
			...
			$lines[] = '<p style="margin: 1em 0px;"><span class="typo3-red">' . $LANG->getLL('errorText') . '</span></p>';
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbsurveyexport/modfunc2/class.tx_pbsurveyexport_modfunc2.php']) {
