...
class tx_pbsurveyexport_modfunc1 extends \TYPO3\CMS\Backend\Module\AbstractFunctionModule
{
    ...
    function init(&$pObj, $conf)
    {
        ...
        $this->intTotalRows = $this->arrResults['finished'] + ($this->arrModParameters['unfinished']
                ? $this->arrResults['unfinished'] : 0);
    }
    ...
    function replaceBlankAnswers()
    {
        foreach ($this->arrCsvRow as $intKey => $strValue) {
            if (is_int($intKey)
                && GeneralUtility::inList('1,23,2,3,4,5', $this->pObj->arrSurveyItems[$intKey]['question_type'])
                && $strValue == ''
            ) {
    ...
    function getSeparator()
    {
        ...
        $this->strSeparator .= $this->arrModParameters['separator']['other']
            ? $this->arrModParameters['separator']['other_value'] : '';
    }
    ...
    function getBlankAnswersProcessing()
    {
        if (isset($this->arrModParameters['replBlank']['bool'])) {
            ...
            $this->strReplacementForBlankAnswers = $this->arrModParameters['replBlank']['value']
                ? $this->arrModParameters['replBlank']['value'] : '0';
        }
    }
    ...
    function writeCsvColumnNames()
    {
        ...
        if (isset($this->arrModParameters['fe_users'])) {
            ...
            foreach ($arrUserKeys as $strColumn) {
                $arrColNames[$strColumn] = preg_replace("/:$/", "",
                    trim($GLOBALS['LANG']->sL($TCA['fe_users']['columns'][$strColumn]['label'])));
            }
        }
        ...
        foreach ($this->pObj->arrSurveyItems as $intQuestionKey => $arrItem) {
            ...
            if (!in_array($arrItem['question_type'], array(10, 12, 13, 14))) {
                if (in_array($arrItem['question_type'], array(1, 2, 3, 4, 5, 23))) {
                    if ($this->arrModParameters['scoring'] != 0
                        && ($arrItem['question_type'] != 1
                            && $arrItem['question_type'] != 23)
                    ) {
                        ...
                        foreach ($arrItem['answers'] as $intAnswerKey => $strAnswer) {
                            $arrColNames[$intQuestionKey . '_' . $intAnswerKey] = $strQuestion . '('
                                . $strAnswer['answer'] . ')';
                        }
                    } else {
                        $strNo = $arrItem['question_type'] == 4 ? $LANG->getLL('value_false')
                            : $LANG->getLL('value_no');
                        $strYes = $arrItem['question_type'] == 4 ? $LANG->getLL('value_true')
                            : $LANG->getLL('value_yes');
                    ...
                    foreach ($arrItem['rows'] as $intRowKey => $row) {
                        if (in_array($arrItem['question_type'], array(6, 7))
                            || ($this->arrModParameters['scoring'] == 0
                                && $arrItem['question_type'] == 8)
                        ) {
                            foreach ($arrItem['answers'] as $intAnswerKey => $answer) {
                                $arrColNames[$intQuestionKey . '_' . $intRowKey . '_' . $intAnswerKey] = $strQuestion
                                    . '(' . $row . ')(' . $answer['answer'] . ')';
                            }
                        } else {
    ...
    function writeCsvResult()
    {
        ...
        if ($this->arrModParameters['rows'] == 'selected') {
            $arrResultsConf['limit'] = $this->arrModParameters['configuration']['from'] . ','
                . $this->arrModParameters['configuration']['count'];
        }

        $dbRes = $GLOBALS['TYPO3_DB']->exec_SELECTquery($arrResultsConf['selectFields'], $this->pObj->strResultsTable,
            $arrResultsConf['where'], '', $arrResultsConf['orderBy'], $arrResultsConf['limit']);
    ...
    {
        $strWrite = GeneralUtility::csvValues($arrInput, $this->strSeparator,
                $this->arrModParameters['separator']['delimiter']) . chr(10);
    ...
    function checkForm()
    {
        ...
        $this->objFileManagement = GeneralUtility::makeInstance(\TYPO3\CMS\Core\Utility\File\BasicFileUtility::class);
        $boolOutput = true;

        if (isset($this->arrModParameters['submit'])) {
            if ($this->arrModParameters['rows'] == 'selected') {
                if ($this->arrModParameters['configuration']['from'] == ''
                    || $this->arrModParameters['configuration']['count'] == ''
                ) {
                    $this->arrError['configuration'] = $LANG->getLL('error_configuration_empty');
                } elseif (!is_numeric($this->arrModParameters['configuration']['from'])
                    || !is_numeric($this->arrModParameters['configuration']['count'])
                ) {
                    $this->arrError['configuration'] = $LANG->getLL('error_configuration_numeric');
                } elseif ($this->arrModParameters['configuration']['from'] < 0
                    || $this->arrModParameters['configuration']['from'] > $this->intTotalRows
                ) {
                    $this->arrError['configuration'] = $LANG->getLL('error_configuration_range');
                }
            }

            if (isset($this->arrModParameters['separator']['other'])
                && !$this->arrModParameters['separator']['other_value']
            ) {
            ...
            if (!$this->arrModParameters['filename']) {
                ...
                $this->arrModParameters['filename']
                    = $this->objFileManagement->cleanFileName($this->arrModParameters['filename']);
            }
        } else {
            $boolOutput = false;
        }
    ...
    function sectionError()
    {
        ...
        if (isset($this->arrError)) {
            ...
            $strTemp .= '<ul class="typo3-red"><li>' . implode('</li>' . chr(13) . '<li>', $this->arrError)
                . '</li></ul>';
    ...
    function sectionConfiguration()
    {
        ...
        $arrOptions[] = '<table>';
        $arrOptions[] = '<tr>';
        $strChecked = (!isset($this->arrModParameters['rows']) || $this->arrModParameters['rows'] == 'all')
            ? 'checked="checked"' : '';
        $arrOptions[] = '<td><input name="' . $this->pObj->strExtKey . '[rows]" type="radio" value="all"' . $strChecked
            . ' /></td>';
        $arrOptions[] = '<td colspan="4">' . $LANG->getLL('export_all') . ' (' . $this->arrResults['finished']
            . ')</td>';
        $arrOptions[] = '</tr>';
        $arrOptions[] = '<tr>';
        $strChecked = ($this->arrModParameters['rows'] == 'selected') ? 'checked="checked"' : '';
        $arrOptions[] = '<td><input name="' . $this->pObj->strExtKey . '[rows]" type="radio" value="selected"'
            . $strChecked . ' /></td>';
        ...
        $arrOptions[] = '<td><input type="text" name="' . $this->pObj->strExtKey . '[configuration][from]" value="'
            . $this->arrModParameters['configuration']['from'] . '" /></td>';
        $arrOptions[] = '<td>' . $LANG->getLL('export_count') . '</td>';
        $arrOptions[] = '<td><input type="text" name="' . $this->pObj->strExtKey . '[configuration][count]" value="'
            . $this->arrModParameters['configuration']['count'] . '" /></td>';
        $arrOptions[] = '</tr>';
        $arrOptions[] = '<tr>';
        $strChecked = (isset($this->arrModParameters['unfinished'])) ? 'checked="checked"' : '';
        $arrOptions[] = '<td><input name="' . $this->pObj->strExtKey . '[unfinished]" type="checkbox" value="1"'
            . $strChecked . ' /></td>';
        $arrOptions[] = '<td colspan="4">' . $LANG->getLL('export_unfinished') . ' (' . $this->arrResults['unfinished']
            . ')</td>';
        $arrOptions[] = '</tr>';
        $arrOptions[] = '</table>';
        $strOutput = $this->pObj->objDoc->section($LANG->getLL('export_configuration'),
            BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_configuration', $GLOBALS['BACK_PATH'],
                '|<br/>') . implode(chr(13), $arrOptions), 0, 1);
    ...
    function sectionSeparator()
    {
        ...
        $arrSeparator[] = '<td><input type="checkbox" name="' . $this->pObj->strExtKey . '[separator][tab]" value="1"'
            . $strChecked . ' /></td><td width="25%">' . $LANG->getLL('separator_tab') . '</td>';
        ...
        $arrSeparator[] = '<td><input type="checkbox" name="' . $this->pObj->strExtKey . '[separator][comma]" value="1"'
            . $strChecked . ' /></td><td width="25%">' . $LANG->getLL('separator_comma') . '</td>';
        $strChecked = (isset($this->arrModParameters['separator']['other'])) ? ' checked="checked"' : '';
        $arrSeparator[] = '<td><input type="checkbox" name="' . $this->pObj->strExtKey . '[separator][other]" value="1"'
            . $strChecked . ' /></td><td width="25%">' . $LANG->getLL('separator_other') . '</td>';
        $strValue = $this->arrModParameters['separator']['other_value'] != ''
            ? $this->arrModParameters['separator']['other_value'] : '';
        $arrSeparator[] = '<td width="25%"><input type="text" name="' . $this->pObj->strExtKey
            . '[separator][other_value]" value="' . $strValue . '" /></td>';
        $arrSeparator[] = '</tr>';
        $arrSeparator[] = '<tr>';
        $strChecked = (isset($this->arrModParameters['separator']['semicolon'])) ? ' checked="checked"' : '';
        $arrSeparator[] = '<td><input type="checkbox" name="' . $this->pObj->strExtKey
            . '[separator][semicolon]" value="1"' . $strChecked . ' /></td><td>' . $LANG->getLL('separator_semicolon')
            . '</td>';
        $strChecked = (isset($this->arrModParameters['separator']['space'])) ? ' checked="checked"' : '';
        $arrSeparator[] = '<td><input type="checkbox" name="' . $this->pObj->strExtKey . '[separator][space]" value="1"'
            . $strChecked . ' /></td><td>' . $LANG->getLL('separator_space') . '</td>';
        $arrSeparator[] = '<td></td><td>' . $LANG->getLL('separator_delimiter') . '</td>';

        if (!isset($this->arrModParameters['submit'])) {
            $strValue = '&quot;';
        } else {
            $strValue = $this->arrModParameters['separator']['delimiter'] != ''
                ? htmlspecialchars($this->arrModParameters['separator']['delimiter']) : '';
        }

        $arrSeparator[] = '<td><input type="text" name="' . $this->pObj->strExtKey . '[separator][delimiter]" value="'
            . $strValue . '" maxlength="1" /></td>';
        $arrSeparator[] = '</tr>';
        $arrSeparator[] = '</table>';
        $strOutput = $this->pObj->objDoc->section($LANG->getLL('separator_options'),
            BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_separator', $GLOBALS['BACK_PATH'],
                '|<br/>') . implode(chr(13), $arrSeparator), 0, 0);
        $strOutput .= $this->pObj->objDoc->divider(10);
    ...
    function sectionUserFields()
    {
        ...
        if (is_array($TCA['fe_users'])) {
            foreach ($TCA['fe_users']['columns'] as $strColName => $arrCol) {
                if(!$arrCol['label']
                    || in_array($strColName,
                        array('usergroup', 'lockToDomain', 'disable', 'starttime', 'endtime', 'TSconfig')))
                {
                    continue;
                }

                $arrUserFields[] = (!$intColCount ? '<tr>' : '') . '<td><input type="checkbox" name="'
                        . $this->pObj->strExtKey . '[fe_users][' . $strColName . ']" value="1" /></td><td width="50%">'
                        . preg_replace("/:$/", "", trim($GLOBALS['LANG']->sL($arrCol['label']))) . '</td>'
                        . ($intColCount ? '</tr>' : '');
        ...
        $arrUserFields[] = '</table>';
        $strOutput = $this->pObj->objDoc->section($LANG->getLL('fe_users'),
            BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_user_information',
                $GLOBALS['BACK_PATH'], '|<br/>') . implode(chr(13), $arrUserFields), 0, 0);
        $strOutput .= $this->pObj->objDoc->divider(10);
    ...
    function sectionBlankAnswers()
    {
        ...
        $strChecked = (isset($this->boolReplaceBlankAnswers) && $this->boolReplaceBlankAnswers) ? ' checked="checked"'
            : '';
        $arrBlankAnswers[] = '<td><input type="checkbox" name="' . $this->pObj->strExtKey
            . '[replBlank][bool]" value="1"' . $strChecked . ' /></td><td>' . $LANG->getLL('blankAnswers_replacement')
            . '</td>';
        $strValue = $this->strReplacementForBlankAnswers != '' ? $this->strReplacementForBlankAnswers : '';
        $arrBlankAnswers[] = '<td><input type="text" name="' . $this->pObj->strExtKey . '[replBlank][value]" value="'
            . $strValue . '" /></td>';
        $arrBlankAnswers[] = '</tr>';
        $arrBlankAnswers[] = '</table>';

        $strOutput = $this->pObj->objDoc->section($LANG->getLL('blankAnswers'),
            BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_blank', $GLOBALS['BACK_PATH'],
                '|<br/>') . implode(chr(13), $arrBlankAnswers), 0, 0);
        $strOutput .= $this->pObj->objDoc->divider(10);
    ...
    function sectionFormat()
    {
        ...
        if ($GLOBALS["TYPO3_CONF_VARS"]["BE"]["forceCharset"] = 'utf-8') {
            ...
            $arrFormat[] = '<table>';
            $arrFormat[] = '<tr><td><input type="radio" name="' . $this->pObj->strExtKey
                . '[chgFormat]" value="0" checked="checked" /></td><td>' . $LANG->getLL('format_utf8') . '</td></tr>';
            $arrFormat[] = '<tr><td><input type="radio" name="' . $this->pObj->strExtKey
                . '[chgFormat]" value="1" /></td><td>' . $LANG->getLL('format_iso') . '</td></tr>';
            $arrFormat[] = '</table>';
            $strOutput = $this->pObj->objDoc->section($LANG->getLL('format'),
                BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_format', $GLOBALS['BACK_PATH'],
                    '|<br/>') . implode(chr(13), $arrFormat), 0, 0);
            $strOutput .= $this->pObj->objDoc->divider(10);
        }
    ...
    function sectionScoring()
    {
        ...
        $arrScoring[] = '<table>';
        $arrScoring[] = '<tr><td><input type="radio" name="' . $this->pObj->strExtKey
            . '[scoring]" value="1" checked="checked" /></td><td>' . $LANG->getLL('scoring_combine') . '</td></tr>';
        $arrScoring[] = '<tr><td><input type="radio" name="' . $this->pObj->strExtKey
            . '[scoring]" value="0" /></td><td>' . $LANG->getLL('scoring_notCombine') . '</td></tr>';
        $arrScoring[] = '</table>';
        $strOutput = $this->pObj->objDoc->section($LANG->getLL('scoring'),
            BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_scoring', $GLOBALS['BACK_PATH'],
                '|<br/>') . implode(chr(13), $arrScoring), 0, 0);
        $strOutput .= $this->pObj->objDoc->divider(10);
    ...
    function sectionSave()
    {
        ...
        $strFileName = $this->arrModParameters['filename'] ? $this->arrModParameters['filename']
            : 'res_' . $strTitle . '_' . date('dmy-Hi') . '.csv';
        $arrFileName[] = '<p>' . $LANG->getLL('save_filename') . '&nbsp;<input type="text" name="'
            . $this->pObj->strExtKey . '[filename]" value="' . $strFileName . '" />&nbsp;<input type="submit" name="'
            . $this->pObj->strExtKey . '[submit]" value="' . $LANG->getLL('save_submit') . '" /></p>';
        $strOutput = $this->pObj->objDoc->section($LANG->getLL('save'),
            BackendUtility::cshItem('_MOD_' . $GLOBALS['MCONF']['name'], 'export_save', $GLOBALS['BACK_PATH'], '|<br/>')
            . implode(chr(13), $arrFileName), 0, 0);
    ...
    function readUser($intInput)
    {
        if (isset($this->arrModParameters['fe_users'])) {
            ...
            if ($intInput) {
                ...
                $arrUserConf['orderBy'] = '';
                $dbRes = $GLOBALS['TYPO3_DB']->exec_SELECTquery($arrUserConf['selectFields'], $this->pObj->strUserTable,
                    $arrUserConf['where'], '', $arrUserConf['orderBy'], '');
    ...
    function readAnswers($intInput)
    {
        ...
        $dbRes = $GLOBALS['TYPO3_DB']->exec_SELECTquery($arrAnswersConf['selectFields'], $this->pObj->strAnswersTable,
            $arrAnswersConf['where'], '', $arrAnswersConf['orderBy'], '');
        ...
        while ($arrAnswersRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($dbRes)) {
            ...
            if (in_array($arrItem['question_type'], array(7, 9, 10, 11, 12, 13, 14, 15, 16))
                || ($arrItem['question_type'] == 1 && $arrAnswersRow['row'] < 0)
                || ($arrItem['question_type'] == 3 && !is_numeric($arrAnswersRow['answer']))
            ) {
...
if (defined('TYPO3_MODE')
    && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pbsurveyexport/modfunc1/class.tx_pbsurveyexport_modfunc1.php']
) {
